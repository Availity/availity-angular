...
const ModalFactory = ($rootScope, $timeout, $compile, $controller, $log, AV_MODAL, avTemplateCache, $q, avModalManager) => {
  ...
  class Modal {
    ...
    listeners() {
      ...
      $element.on(AV_MODAL.BS_EVENTS.SHOW, event => {
        scope.$emit(AV_MODAL.EVENTS.SHOW, event, self);
      });

      $element.on(AV_MODAL.BS_EVENTS.SHOWN, event => {
      ...
      $element.on(AV_MODAL.BS_EVENTS.HIDE, event => {
    ...
    toggle() {
      return this.templatePromise.then(() => this.isShown() ? this.hide() : this.show());
    }
