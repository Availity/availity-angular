...
(function(root) {
  ...
  availity.core.provider('avIdle', function() {
    ...
    this.$get = function(AV_IDLE, $log, $document, $rootScope, $timeout, avThrottle, $q, $injector) {
      ...
      proto.onSessionTimerExpire = function() {
        $log.info("idle session");
        this.onDisabled();
        $rootScope.$broadcast(AV_IDLE.EVENTS.SESSION_TIMEOUT);
