...
class AvPollingService extends Base {
  ...
  isAsyncResponse(response) {
    ...
    return response &&
      ...
      response.status &&
      response.status === 202 &&
      angular.isFunction(response.headers) && !utils.isBlank(response.headers(this.av.AV_API.HEADERS.SERVER.LOCATION));
  }
  ...
  getUrl(url) {
    ...
    const result = url.match(this.avAV_POLLING.REGEX_URL);
    if (result && result[1]) {
      return `/api${result[1]}`;
    }
  ...
  queueRequest(deferred, response) {
    ...
    const _url = utils.getRelativeUrl(response.headers(this.av.AV_API.HEADERS.SERVER.LOCATION));
