  ...
  availity.core.factory('avValDateRange', function(AV_VAL, avValUtils) {
    ...
    var validator = {
      ...
      validation: function(value, rules) {
        var selectedDate,minDate,maxDate;

        selectedDate = moment(value, rules.dateFormat);
        if(rules.type === 'dd'){
          minDate = moment(rules.min,'MMDDYYYY').subtract(1, 'Day');
          maxDate = moment(rules.max,'MMDDYYYY').add(1,'Day');
        }else{
          minDate = validator.getMinDate(rules.min);
          maxDate = validator.getMaxDate(rules.max);
          
        }
        return selectedDate.isBetween(minDate,maxDate);;
      },
