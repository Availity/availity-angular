...
describe('AvApiResourceProvider', function() {
  ...
  beforeEach(module('availity', function(_AV_API_, _AvApiResourceProvider_, _$httpProvider_, $provide) {

    AV_API = _AV_API_;

    provider = _AvApiResourceProvider_;
    $httpProvider = _$httpProvider_;
    // Push default header to common
    provider.setHttpCommonHeaders($httpProvider);
    // now override the defaults
    provider.setOptions({
      headers: {
        // Turn on content encoding for angular apis
  ...
  it('should have changed common http headers', function() {
    // Should be NONE since changed before changed to HTML
    expect($httpProvider.defaults.headers.common[AV_API.HEADERS.CLIENT.RESPONSE_ENCODING]).toBe('NONE');
  });
  ...
  describe('AvApiResource', function() {
    ...
    describe('_config', function() {
      it('should be default options', function() {
        ...
        var config = users._config();
        expect(config.headers[AV_API.HEADERS.CLIENT.RESPONSE_ENCODING]).toBe('HTML');
        var expectedOptions = _.merge({}, provider.getOptions(), {url: '/users'});
        expect(config).toBeEqual(expectedOptions);
      });
      ...
      it('should merge headers', function() {
        ...
        var config = users._config(options);
        expect(config.headers[AV_API.HEADERS.CLIENT.CUSTOMER_ID]).toBe('1194');
        expect(config.headers[AV_API.HEADERS.CLIENT.RESPONSE_ENCODING]).toBe('HTML');
        var expectedOptions = _.merge({}, provider.getOptions(), {url: '/users'}, options);
        expect(config).toBeEqual(expectedOptions);
      });

      it('should overwrite encoding', function() {
        var users = new AvApiResource('users');
        var options = {
          headers: {
            'X-Availity-Customer-ID': '1194',
        ...
        var config = users._config(options);
        expect(config.headers[AV_API.HEADERS.CLIENT.CUSTOMER_ID]).toBe('1194');
        expect(config.headers[AV_API.HEADERS.CLIENT.RESPONSE_ENCODING]).toBe('NONE');
        var expectedOptions = _.merge({}, provider.getOptions(), {url: '/users'}, options);
        expect(config).toBeEqual(expectedOptions);
      });
    ...
    describe('rest', function() {
      ...
      describe('all()', function () {
        ...
        beforeEach(function() {
          $httpBackend.expectGET('/api/v1/users').respond(200, pagedResponses[0]);
          $httpBackend.expectGET('/api/v1/users?limit=50&offset=50').respond(200, pagedResponses[1]);
          $httpBackend.expectGET('/api/v1/users?limit=50&offset=100').respond(200, pagedResponses[2]);
        });

        it('should get list of resources', function() {

          users.all().success(function(data) {
            expect(data).toBeEqual(['page 1', 'page 2', 'page 3']);
          });
