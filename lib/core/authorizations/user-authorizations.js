(function(root) {
   ...
   * @constructor
   */
  function AvUserAuthorizations(permissionCache) {
  ...
  AvUserAuthorizations.prototype.getUserPermission = function(permissionId) {
    ...
    var permissionLoader = function() {
      var success = function(permissions) {
        ...
        return self._clonePermission(permission);
      };
      var invalidateUser = self.invalidateUser; // copy current value local
      self.invalidateUser = false; // set to false for all furture calls
      return avUserPermissionsResource.getPermissions(permissionId, self.region, invalidateUser).then(success);
    };
    return self.permissionCache.get(permissionId, permissionLoader);
  };
  ...
  availity.core.factory('avUserAuthorizations', function($injector) {
    ...
    return new AvUserAuthorizations($injector.get('avCache').getCache('avUserAuthorization.permissionCache'));
  });
