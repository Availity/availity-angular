  ...
  availity.core.provider('avPiwikAnalytics', function() {
    ...
    this.$get = function(avAnalyticsUtils, avUsersResource, AV_ANALYTICS, $injector, $log, $q, $document, $location, $window) {
      ...
      proto.trackPageView  = function(url) {
        ...
        // must also push 'setCustomUrl'
        // in the visitorLog unless the user changes location via a page load the url doesn't update
        // the pageTitle references the correct page url
        return $q.when(root._paq.push(['trackPageView', url]));
