(function(root) {
  ...
  availity.core.provider('avPiwikAnalytics', function() {
    ...
    this.$get = function(avAnalyticsUtils, avUsersResource, AV_ANALYTICS, $injector, $log, $q, $document, $location, $window) {
      ...
      proto.trackPageView  = function(url) {
        ...
        return $q.when([root._paq.push(['setCustomUrl', url]),root._paq.push(['trackPageView', url])]);
  ...
  availity.core.run(function($rootScope, AV_ANALYTICS, avAnalytics, avPiwikAnalytics, $location ) {

    if(avPiwikAnalytics.isEnabled() && avAnalytics.getVirtualPageTracking() ) {
