(function(root) {
  ...
  availity.core.provider('avAnalytics', function(AV_ANALYTICS) {
    ...
    this.getVirtualPageTracking = function() {
    ...
    this.$get = function($injector, $q, $log) {
      ...
      var AvAnalytics = function() {
        ...
        angular.forEach(plugins, function(plugin) {
          ...
          try {
            AvAnalytics.services[plugin] = $injector.get(plugin);
          } catch(err) {
      ...
      proto.trackEvent = function(properties) {
        ...
        angular.forEach(AvAnalytics.services, function(handler) {
      ...
      proto.getVirtualPageTracking = function() {
        return virtualPageTracking;
      };
  ...
  availity.core.run(function($rootScope, AV_ANALYTICS, avAnalytics, $location ) {
    ...
    if(avAnalytics.getVirtualPageTracking()) {
    ...
    angular.forEach(avAnalytics.prototype.constructor.services, function(handler) {

      if(handler.isEnabled() && handler.createScript) {
