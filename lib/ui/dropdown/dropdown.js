(function(root) {
  ...
  availity.ui.controller('AvDropdownController', function($element, $attrs, AV_UI, AV_DROPDOWN, avDropdownConfig, $log, $scope, $timeout, $parse) {
    ...
    this.init = function() {
      ...
      if(angular.isDefined($attrs.multiple) && $element.get(0).tagName.toLowerCase() !== 'select') {
    ...
    this.getMultiSelected = function(viewValue) {
      ...
      if($element.get(0).tagName.toLowerCase() !== 'input') {
        ...
        _.each(inputViewValues, function(savedObject) {
          ...
          if(!savedObject.id && !_.isEmpty(savedObject[self.options.idSubstitute])) {

            savedObject.id = savedObject[self.options.idSubstitute];

          } else {
            throw new Error('dropdown list must have a id or a alternative value to use as a id');
          }

          indices.push(savedObject.id);
        ...
        element.on('change', function(e) {
          ...
          if(avDropdown.options.query) {
            $timeout(function() {
              ...
              if(element.get(0).tagName.toLowerCase() === 'input' && attrs.multiple) {

                var index;

                if(!angular.isArray(ngModel.$viewValue) || !angular.isObject(ngModel.$viewValue)) {
                ...
                if(e.added) {
                  ...
                  ngModel.$viewValue.push(e.added);

                } else {

                  index = _.findIndex(ngModel.$viewValue, function(value) {
