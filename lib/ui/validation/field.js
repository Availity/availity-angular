...
class AvValFieldController extends Base {
  ...
  validate(modelValue, viewValue) {

    const value = modelValue || viewValue;

    this.av.$log.info(`validating value [${value}]`);

    const rulesSchema = this.avValForm.rulesSchema;
    const results = this.av.avVal.validate(rulesSchema, this.av.$element, value, this.rule);
