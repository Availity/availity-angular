...
describe('avAnalytics', function() {
  ...
  it('should track event', function () {
    $el = availity.mock.compileDirective('<div data-av-analytics><div data-av-analytics-on="click"></div></div>');
    var controller = $el.data('$avAnalyticsController');
  ...
  it('should override default options', function () {
    $el = availity.mock.compileDirective('<div data-av-analytics="{abc:\'123\', test:\'test\'}"><div data-av-analytics-on="Play"></div></div>');
    var controller = $el.data('$avAnalyticsController');
    ...
    availity.mock.flush();
    expect(controller.onEvent).toHaveBeenCalled();
    expect(avAnalytics.trackEvent).toHaveBeenCalledWith({ event: 'Play', level: 'info', abc: '123', test: 'test'});
  });
