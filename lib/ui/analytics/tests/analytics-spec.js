...
describe('avAnalytics', function() {
  ...
  it('should allow angular values', function () {
  ...
  it('should keep initial angular value', function () {
    availity.mock.$scope.label = "Testing";
    $el = availity.mock.compileDirective('<div><div data-av-analytics-on="click" data-av-analytics-label="{{label}}"></div></div>');
    var controller = $el.children().data('$avAnalyticsOnController');
  ...
  it('should allow dynamic values', function () {
    availity.mock.$scope.label = "Testing";
    $el = availity.mock.compileDirective('<div><div data-av-analytics-on="click" data-av-analytics-label="{{label}}" data-av-analytics-dynamic="label"></div></div>');
    var controller = $el.children().data('$avAnalyticsOnController');
    availity.mock.$scope.label = "Testing2";
    availity.mock.$scope.$digest();
    spyOn(controller, 'onEvent').and.callThrough();
    ...
    availity.mock.flush();
    expect(controller.onEvent).toHaveBeenCalled();
    expect(avAnalytics.trackEvent).toHaveBeenCalledWith({ event: 'click', level: 'info', label: 'Testing2', dynamic: 'label'});
  });
